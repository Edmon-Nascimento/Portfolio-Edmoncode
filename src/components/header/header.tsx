...
export default function Header() {
  ...
  return (
    <header className="sticky top-0 z-40 flex items-center justify-between py-5 px-6 sm:px-8 bg-primary/95 backdrop-blur-md border-b border-accent/10">
      ...
      >
        <h1 className="text-xl sm:text-2xl md:text-3xl font-bold tracking-tight text-white transition-colors group-hover:text-accent/90">
          EDMON<span className="text-accent">CODE</span>
        </h1>
      </a>
      ...
      <button
        ...
        className="flex items-center justify-center w-12 h-10 z-50 p-2 rounded-lg text-accent hover:bg-accent/10 focus:outline-none focus-visible:ring-2 focus-visible:ring-accent focus-visible:ring-offset-2 focus-visible:ring-offset-primary transition-colors lg:hidden md:w-15"
      >
      ...
      <nav
        id="mobile-menu"
        className={`fixed top-0 left-0 h-screen w-64 sm:w-72 bg-primary border-r border-accent/10 shadow-xl shadow-black/30
        ...
        <ul className="flex flex-col gap-1 lg:flex-row lg:items-center lg:gap-8">
          {navLinks.map(({ href, label }) => (
            <li key={label}>
              <a
                href={href}
                className="block py-3 px-4 rounded-lg text-[#e8e8e8] text-base md:text-lg hover:text-accent hover:bg-accent/10 transition-colors duration-200 font-medium lg:py-2 lg:px-0 lg:bg-transparent lg:hover:bg-transparent lg:relative lg:after:content-[''] lg:after:absolute lg:after:bottom-0 lg:after:left-1/2 lg:after:-translate-x-1/2 lg:after:h-0.5 lg:after:bg-accent lg:after:w-0 lg:hover:after:w-full lg:after:transition-all lg:after:duration-200"
